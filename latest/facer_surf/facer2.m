...
% Last Modified by GUIDE v2.5 04-Jul-2017 13:58:47
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global A m1 n1 No_Files_In_Class_Folder Class_Count Training_Set_Folder surf_names
...
points1 = detectSURFFeatures(imread(test_image_path));
Test_File = [Test_File_Path Test_File];
test = imread(Test_File);
name1 = getnames(Test_File);
set(handles.txt_ipimage,'string',name1);
if length(size(test))==3
...
points2 = detectSURFFeatures(Test_Image);
...
bpnnout  = bpnn(points1,points2,n,y);
...
disp(Image_Path);
axes(handles.axes2);
name2 = getnames(Image_Path);
set(handles.txt_opimage,'string',name2);
imshow(Class_Image);
points2 = detectSURFFeatures(imread(Class_Image));
[features1,valid_points1] = extractFeatures(imread(test_image_path),points1);
[features2,valid_points2] = extractFeatures(imread(Class_Image),points2);
...
match(test_image_path,Class_Image);
